...
<script>
    ...
    import {errorMsg, loadingOpen, loadingClose} from "../../../plugins/tool";
    ...
    export default {
        ...
        methods: {
            netPay_like() {
                ...
                loadingOpen("支付发起中！")
                let _this = this
                XNetPay({'uid': _this.uid, "amount_of_account": _this.amount_of_account}).then(function (data) {
                    loadingClose()
                    if(data.code == "0000") {
                        let credential = data.data.result.JWP_ATTR
                        paymentjs.createPayment(credential, function (result, err) {
                        })
                        _this.dialogCharging = true
                    } else {
                        if(data.msg) {
                            errorMsg(data.msg)
                        }else{
