        ...
        <div class="from">
            <el-form :model="ruleForm" ref="ruleForm" :rules="rules" label-width="106px" class="demo-ruleForm">
                ...
                <el-form-item label="设置提现密码" prop="password">
                    <el-input type="password" v-model="ruleForm.password" auto-complete="off" placeholder="请设置提现密码"></el-input>
                </el-form-item>
            </el-form>
            <p>为了账户安全，请不要设置与登录密码相同的提现密码</p>
...
<script>
import {getBackWithdrawPassword, getSms} from '~/api/personal.js';
...
export default {
    ...
    methods: {
        ...mapMutations(['changeSubmitting', 'isLoginFlag']),
        getcaptCode(obj) {
                ...
                if (valid) {
                    ...
                    .then((res) => {
                        if (res.status == 1) {
                            ...
                            _this.$router.push({name: 'mine-account', query:{tabView: 'select2', iscur: 1}})
                        } else {
                            errorMsg(res.msg)
                        }
                    })
